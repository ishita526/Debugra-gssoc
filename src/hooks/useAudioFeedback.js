...
export function useAudioFeedback() {
  ...
  const playFallbackTone = useCallback((outcome) => {
    const context = getAudioContext();
    if (!context) return;
    ...
    window.setTimeout(() => {
      ...
      activeNodesRef.current = activeNodesRef.current.filter((node) => node.context.currentTime < now + 0.45);
    }, 500);
  }, [getAudioContext, prepare, settings.muted, settings.volume, stopActiveNodes]);

  const playOutcome = useCallback((outcome) => {
    ...
    if (!audio) {
      console.debug('[audio] static asset missing for', outcome, '— falling back to synthesized tone');
      console.debug('[audio] static asset missing for', outcome, '— falling back to synthesized tone');
    ...
    if (playPromise?.then) {
      playPromise.then(() => {
        ...
        try { toast.error('Audio playback blocked — check tab/system volume'); } catch (e) {}
        playFallbackTone(outcome);
      });
    }
